...
%v1 Written by Adam Rauff and Chelsea Heveran

% "backend" of the Quality control GUI
% This GUI is generated with MATLAB's GUIDE

% This script is parsed by buttons and functions
% Part 1 
% Initialization
...
% Part 4
% User Click Event
% User clicks on image to exclude objects manually

% Part 5
% Veiwing and Rendering
...
% --- Executes just before Image_Process_GUIDE (GUI) is made visible.
...
%% part 2 - exit program Returned Variables 
...
%% Part 4 - User Click Event
% executes when User clicks on image
...
% uncomment to print the coordinates found and the value (intensity) of
% that voxel
% disp(['coordinates: ', num2str([x_coord y_coord])]);
% disp(['Slice: ',num2str(Value)]);

% obtain lacunae belonging to clicked pixel
LabeldIM = handles.InputStruct(1).ImageSet;
objVal = LabeldIM(y_coord-6,x_coord-3,Value); %slight offset from clicked value. Experimentally determined
...
if objVal ~= 0
    ...
    % convert linear indices to subscripts (row, col, dim)
    [~, ~, dim] = ind2sub(size(LabeldIM),LinInds); 
    
    %identify the slice range (depth) where object exists
...
%% Part 5 - veiwing and rendering tools
...
% Hard to evaluate if red (segmented image) goes beyond lacuna trace
...
%% Part 6 - Revert to previous configuration (Undo button)
